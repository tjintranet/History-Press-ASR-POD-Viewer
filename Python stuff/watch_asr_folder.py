...
def setup_logging(log_file: Path, no_console: bool = False) -> None:
    ...
    # Console handler — omit when running headless via Task Scheduler
    if not no_console:
...
def main():
    ...
    # Register a clean shutdown handler for signals that Windows / Task Scheduler
    # may send when stopping the task (SIGTERM) or when the console is closed
    # (SIGBREAK on Windows).
    ...
    # SIGBREAK is Windows-only (Ctrl+Break / taskkill); guard with hasattr
